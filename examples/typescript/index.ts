...
async function runActor() {
  ...
  const actorId = `lexis-solutions/redfin-scraper`;
  console.log(`Starting actor: ${actorId}`);

  const { defaultDatasetId } = await client.actor(actorId).call({
    startUrls: [
      {
        "url": "https://www.redfin.com/city/11203/CA/Los-Angeles/filter/property-type=house,min-price=80M,max-price=150M,min-beds=5,min-baths=4,school-types=elementary+middle+high"
      }
    ],
    mode: "buy",
    maxPages: 1,
